...
if __name__ == '__main__':
    ...
    save_dir = "/Users/vl/Documents/Kuchipudi_Thesis/regression"

    # Create the directory if it doesn't exist
    os.makedirs(save_dir, exist_ok=True)

    # Extract day 1 and day 40 data for category 4 for each column
    day1_cat4 = {col: data_for_reg[col][data_for_reg["Category"].index(4)] for col in data_for_reg.keys() if
                 col != "Category" and col != "Day"}
    day40_cat4 = {col: data_for_reg[col][5] for col in data_for_reg.keys() if col != "Category" and col != "Day"}


    # Define a function to interpolate values between day 1 and day 40 for categories 1, 2, and 3
    def interpolate_values(day1, day40, day40_cat4):
        return [day1 + (day40_cat4 - day1) * (i / 40) for i in range(1, 41)]


    # Interpolate values for categories 1, 2, and 3 for each column
    interpolated_data = {}
    for col in day1_cat4.keys():
        interpolated_data[col] = interpolate_values(data_for_reg[col][1], data_for_reg[col][5], day40_cat4[col])

    # Plot and save each feature separately
    plt.figure(figsize=(16, 12))
    for i, (col, values) in enumerate(interpolated_data.items(), 1):
        plt.subplot(3, 3, i)
        plt.plot(range(1, 41), values, label=col)
        plt.axhline(y=day40_cat4[col], color='r', linestyle='--', label=f"{col} (Category 4, Day 40)")
        plt.xlabel('Day')
        plt.ylabel('Values')
        plt.title(f'Linear Interpolation for {col} with Comparison to Category 4')
        plt.legend()
        plt.grid(True)

    # Save the consolidated plot as PNG
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'all_columns.png'))
    plt.show()
